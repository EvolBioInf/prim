...
this parameter to the number of expected accessions, but that would
preclude finding any hits beyond this set. So we give ourselves a
three-fold safety margin, but also allow the user to increase or
decrease this value. If you are surprised by low sensitivity, try
increasing \ty{-m} and you might observe a reduction in false
negatives, and hence an increase in the sensitivity.
#+end_export
...
(\ty{-t})---have been set. If so, we look up the expected target
accessions and respond to the number of threads, (\ty{-T}).
#+end_export
...
``complete genomes'' that are not ``plasmids''.
#+end_export
...
(Table~\ref{tab:bout}). Once constructed, we run the Blast command,
...
expected. We also save these accessions.
#+end_export
